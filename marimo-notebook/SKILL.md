...
description: >
  This skill should be used when the user asks to "create a marimo notebook", "write a
  marimo app", "build an interactive dashboard with marimo", "add cells to a notebook",
  "deploy a marimo app", or "debug marimo reactivity". Also use when working with marimo
  .py notebook files, discussing reactive cells, UI elements (sliders, dropdowns, tables),
  or marimo-specific patterns like mo.ui, mo.md, mo.sql — even if the user does not
  explicitly mention marimo by name. Covers cell structure, reactivity, PEP 723
  dependencies, script mode, testing with pytest, and deployment.
---
